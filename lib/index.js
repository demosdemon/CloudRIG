...
const instanceTypes = ["g2.2xlarge", "g3s.xlarge", "g3.4xlarge"];
const instanceTypesFiltering = {
	"eu-west-2": {
		"g2.2xlarge": false
	}
}
...
let githubContributorsCache = null;
const githubActiveMaintainersLogins = ['NitriKx'];
const githubOriginalDevelopmentMaintainersLogins = ['williamparry'];
function getGithubContributors(cb) {
	logger.debug("Loading contributors...");
	if (!githubContributorsCache) {
		request({
			url: 'https://api.github.com/repos/cloudrig/cloudRIG/contributors',
			headers: {
				'User-Agent': 'cloudRIG-App'
			}
		},
		function (error, response, body) {
			if (error) {
				logger.error("Could not load the contributor list: " + error);
				cb(error)
			} else {
				const githubContributors = JSON.parse(body);
				logger.debug("Found " + githubContributors.length + " contributors");

				// Extract the active maintainers list
				const activeMaintainers = githubContributors.filter(function(contributor) {
					return contributor.login && githubActiveMaintainersLogins.indexOf(contributor.login) >= 0;
				});

				// Extract the original development maintainers list
				const originalDevelopmentMaintainers = githubContributors.filter(function(contributor) {
					return contributor.login && githubOriginalDevelopmentMaintainersLogins.indexOf(contributor.login) >= 0;
				});

				// Get all the others
				const otherContributors = githubContributors.filter(function(contributor) {
					return contributor.login && githubActiveMaintainersLogins.indexOf(contributor.login) < 0
																	 && githubOriginalDevelopmentMaintainersLogins.indexOf(contributor.login) < 0
				});

				githubContributorsCache = {
					originalList: githubContributors,
					activeMaintainers: activeMaintainers,
					originalDevelopmentMaintainers: originalDevelopmentMaintainers,
					otherContributors: otherContributors
				};

				cb(null, githubContributorsCache)
			}
		});
	} else {
		logger.debug("Using contributor list cache");
		cb(null, githubContributorsCache)
	}
}
	...
	ec2.describeImages(
		{
			Filters: [
				{
					Name: "description",
					Values: ["Parsec Server"],
				},
				{
					Name: "name",
					Values: [config.AWSInstanceType == "g2.2xlarge" ? "g2.2xlarge-*" : "g3.4xlarge-*"],
				}
			]
		},
		function(err, data) {
		...
		getAMI(function(err, ami) {
			...
			if (ami) {
				...
				startFunctions = [
					...
					_processEBSVolume,              // Resize-Drive.ps1

					_initialiseEphemeralDisks,      // Initialise-Ephemeral-Disks.ps1

					_reboot,

					_clearPreviousScheduledTasks,   // Cancel-Scheduled-Shutdown.ps1

					_createShutdownNotifier,        // Create-Shutdown-Notification.ps1

					_createTerminationChecker,      // Create-Termination-Checker.ps1

					_scheduleReset,                 // Schedule-Reset.ps1

					_addTerminationChecker,         // Schedule-Termination-Checker.ps1
				...
				startFunctions = [
					...
					_installFolders,               // Install-Folders.ps1

					_createShutdownNotifier,       // Create-Shutdown-Notification.ps1

					_createTerminationChecker,     // Create-Termination-Checker.ps1

					_initialiseDrive,              // Set-Drive.ps1
...
module.exports = {
	...
	getGithubContributors: getGithubContributors,
